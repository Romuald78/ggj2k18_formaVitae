        ...
        for(int entIdx=0;entIdx<entList.size();entIdx++)
        {
            ...
            for(int cmpIdx=0;cmpIdx<cmpList.size();cmpIdx++)
            {
                ...
                if(cmpList.get(cmpIdx).getClass() == AnalogPadHandler.class)
                {
                    ...
                    // Update pad position
                    int idAxisX = ControllerButtons.getAxisIDFromAxisName(ctrlID, ControllerButtons.AXIS_ANALOG_X);
                    int idAxisY = ControllerButtons.getAxisIDFromAxisName(ctrlID, ControllerButtons.AXIS_ANALOG_Y);
